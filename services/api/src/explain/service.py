...
7. 参考真实老师白板讲解：一屏一个核心想法；顶部蓝色手写标题；主体图居中，占画面 45%-65%；保留大面积空白；文字只写短标签和关键结论。
8. 画面不是信息图海报：不要密集段落、图例框、装饰面板、彩色背景块、卡片式布局或复杂 UI；所有内容都像老师在白板上逐步补出来。
9. 色彩有教学含义：黑色主体线，蓝色标题/控制关系，红色风险/电流/错误，绿色有效路径/正确结果，黄色只做小下划线或局部强调。
10. 复杂图不要强行逐笔画完：如果一张图过密、标签多、像参考图/成品图/多层结构，应建议直接呈现主体，再用少量手写箭头、圈、下划线和局部 callout 讲重点。
11. 不要把所有视频都做成同一种“手一直画完整图”。根据内容选择呈现策略：
...
12. 对每个场景明确推荐 `board_mode`、`hand_usage`、`visual_style`。这不是模板库，而是课堂表达语法。
13. 不要围绕某个题材词做特殊处理；先抽象出教学关系，再选择画面语法：
...
STYLE_STRATEGY_RULES = [
    ...
    "visual_style=teacher_whiteboard：稀疏白板课堂；marketing_doodle：彩色成品 doodle + 手部强调；math_chalkboard：黑板彩色粉笔；technical_reference：复杂主体 + 精准局部讲解。",
]
...
def _ensure_brief_minimums(brief: EnhancedTeachingBrief, req: GenerateGraphRequest) -> EnhancedTeachingBrief:
    ...
    if any(signal in blob_lower for signal in math_signals):
