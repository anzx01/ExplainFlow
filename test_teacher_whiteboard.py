...
for i, scene in enumerate(sb['scenes'][:3]):
    ...
    print('  Scene %d: visualStyle=%s, handUsage=%s' % (i, vs, hu))
print()
...
for i in range(120):
    time.sleep(5)
    try:
        req4 = urllib.request.Request(RENDER_BASE + '/render/jobs/' + job_id, headers={'Content-Type': 'application/json'})
        ...
        if status.get('status') == 'done':
            ...
            video_url = RENDER_BASE + '/render/jobs/' + job_id + '/download'
            req5 = urllib.request.Request(video_url)
            with urllib.request.urlopen(req5, timeout=60) as r:
                video_data = r.read()
            output_path = Path(__file__).resolve().parent / 'outputs' / 'teacher_whiteboard_test.mp4'
